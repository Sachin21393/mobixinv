...
const vaccine_database = new Datastore("./database/vaccineregister.db");
const hospital_database = new Datastore("./database/hospitalreg.db");
const agent_database = new Datastore("./database/agentreg.db");

// const inventory_database = new Datastore("./database/inventory.db");
user_database.loadDatabase();
vaccine_database.loadDatabase();
hospital_database.loadDatabase();
agent_database.loadDatabase();

// inventory_database.loadDatabase();
const databaseTable = {
user: user_database,
listt: agent_database,
...
app.get("/qr", (req, res) => {
  const params = {};
  res.status(200).render("qr.pug", params);
});
app.get("/register", (req, res) => {
  const params = {};
  res.status(200).render("register.pug", params);
});
app.get("/customersignin", (req, res) => {
  const params = {};
  res.status(200).render("customersignin.pug", params);
});
app.get("/customerregister", (req, res) => {
  const params = {};
  res.status(200).render("customerregister.pug", params);
});
...
app.post("/login", async (req, res) => {
  ...
  user_database.findOne({ email: email }, (err, data) => {
    try {
      ...
      console.log(data);
      if (data.password === password) {
        res.status(200).render("rehome.pug");
      } else {
        res.send("invalid");
      }
    } catch {
